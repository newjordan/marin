...
def get_claim_for_worker(tx: Tx, worker_id: WorkerId) -> tuple[str, int] | None:
    ...
    row = tx.execute(
        select(reservation_claims_table.c.job_id, reservation_claims_table.c.entry_idx).where(
...
def list_claims_for_job(tx: Tx, job_id_wire: str) -> list[tuple[WorkerId, int]]:
    ...
    rows = tx.execute(
        select(reservation_claims_table.c.worker_id, reservation_claims_table.c.entry_idx).where(
...
def get_last_submission_ms(tx: Tx) -> int:
    ...
    row = tx.execute(select(meta_table.c.value).where(meta_table.c.key == "last_submission_ms")).first()
