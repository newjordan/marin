            ...
            for ch in surface:
                if pending_sentinel:
                    if ch == sentinel:
                        ...
                        raise LosslessCapsError(f"invalid continuation {ch!r} after capitalization sentinel")
                    pending_sentinel = False
                    continue
                if ch == sentinel:
    ...
    for surface in surfaces:
        ...
        for ch in surface:
            ...
            if ch == esc:
                ...
                continue
            if (
                normalized in {LOSSLESS_CAPS_V2, LOSSLESS_CAPS_V3, LOSSLESS_CAPS_V5, LOSSLESS_CAPS_CASEOPS_V1}
                and ch == title
            ):
                if pending_word_mode is not None or in_ascii_word or pending_capnext:
                    raise LosslessCapsError("invalid title marker placement")
